...
for iblock = 1:nBlocks
    ...
    for iframe=1:nFrames
        ...
        W_track_left_t = conj(mvdr_fcn(R,rH_track_left_t));
        W_track_right_t = conj(mvdr_fcn(R,rH_track_right_t));
        ...
        % apply
        Y_track_left(:,:,iframe) = sum(bsxfun(@times,X_cola(:,:,iframe),W_track_left),2);
        Y_track_right(:,:,iframe) = sum(bsxfun(@times,X_cola(:,:,iframe),W_track_right),2);
    end   
